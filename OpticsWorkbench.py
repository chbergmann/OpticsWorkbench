...
def makeAbsorber(base = [], collectStatistics = False, transparency=0, reflectionRate=0):
    #reload(OpticalObject)
    '''All FreeCAD objects in base will be optical light absorbers.'''
    fp = activeDocument().addObject('Part::FeaturePython', 'Absorber')
    OpticalObject.OpticalObjectWorker(fp, base, type = 'absorber', collectStatistics = collectStatistics, transparency = transparency, reflectionRate = reflectionRate) 
    OpticalObject.OpticalObjectViewProvider(fp.ViewObject)
    recompute()
    return fp

def makeLens(base = [], RefractionIndex = 0, material = 'Quartz', collectStatistics = False, transparency=100, reflectionRate=0):
    #reload(OpticalObject)
    '''All FreeCAD objects in base will be optical lenses.'''
    fp = activeDocument().addObject('Part::FeaturePython', 'Lens')
    OpticalObject.LensWorker(fp, base, RefractionIndex, material, collectStatistics, transparency = transparency, reflectionRate = reflectionRate)
    OpticalObject.OpticalObjectViewProvider(fp.ViewObject)
    recompute()
    return fp
